...
public class SimulationDisplay : MonoBehaviour
{
    ...
    public void RunSimulation()
    {
        ...
        OnSimulationRunComplete(fishCaught, hours, cash);
    }

    void Start()
    {
        //TrySubscribeToSimulation();
        RunSimulation();
    }
    ...
    void TrySubscribeToSimulation()
    {


        //Simulation = Object.FindAnyObjectByType<FishSimulation>();
        //if (Simulation != null)
        //{
        //    Simulation.OnRunComplete += OnSimulationRunComplete;
        //    Refresh(0, 0, 0);
        //}
    }
    ...
    void OnSimulationRunComplete(int fish, int hours, int cash)
    {
        Refresh(fish, hours, cash);
        
    }

    void Refresh(int fish, int hours, int cash)
    {
        if (fishCaughtText != null)
        {
            fishCaughtText.text = Simulation != null ? fish.ToString() : "N/A";
        }
        if (hoursText != null)
        {
            hoursText.text = Simulation != null ? hours.ToString() : "N/A";
        }
        if (cashText != null)
        {
            cashText.text = Simulation != null ? cash.ToString() : "N/A";
        }
